...
- (void)viewDidLoad {
    ...
    _doneButton.frame  = CGRectMake(0, 0, 60, 35);
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:tzImagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(doneButtonClick)];
...
//    _previewButton.enabled = tzImagePickerVc.selectedModels.count > 0;
    ...
    if (_doneButton.enabled == YES) {
        [_doneButton setTitle:[NSString stringWithFormat:@"完成(%ld)",tzImagePickerVc.selectedModels.count] forState:UIControlStateNormal];
    }else{
