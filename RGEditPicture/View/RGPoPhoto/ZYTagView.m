...
#define kTagVerticalSpace 8.0                   /** 标签上下空余距离 */
#define kPointWidth 6.0                          /** 白点直径 */
...
- (void)createSubviews
{
    ...
    backLayer.fillColor = [[UIColor whiteColor] colorWithAlphaComponent:1.f].CGColor;
    ...
    titleLabel.backgroundColor = HEXCOLORAL(0x999999, 0.8);
    [self addSubview:titleLabel];
    self.titleLabel = titleLabel;
    
    UIButton *deleteBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//    deleteBtn.backgroundColor = HEXCOLORAL(0x999999, 0.8);
    [deleteBtn addTarget:self action:@selector(clickDeleteBtn) forControlEvents:UIControlEventTouchUpInside];
    [deleteBtn setImage:[UIImage imageNamed:@"rg_editPhot_deleteTag"] forState:UIControlStateNormal];
...
- (void)layoutWithTitle:(NSString *)title superview:(UIView *)superview
{
    ...
    self.titleLabel.layer.cornerRadius = MIN(self.titleLabel.zy_width, self.titleLabel.zy_height)/2.f;
...
- (void)layoutSubviewsWithState:(ZYTagViewState)state arrowPoint:(CGPoint)arrowPoint
{
    ...
    if (state == ZYTagViewStateArrowLeft || state == ZYTagViewStateArrowLeftWithDelete) {
        ...
        self.zy_x = arrowPoint.x - kPointWidth / 2.0;
        // 背景
        backPath.lineWidth = 1.f;
        backPath.lineCapStyle = kCGLineCapRound;
        backPath.lineJoinStyle = kCGLineCapRound;
        [backPath moveToPoint:CGPointMake(kPointWidth + kPointSpace-1, self.zy_height / 2.0-0.5f)];
        [backPath addLineToPoint:CGPointMake(kPointWidth + kPointSpace-1, self.zy_height / 2.0+.5f)];
        [backPath addLineToPoint:CGPointMake(kPointWidth + kPointSpace+ kAngleLength-1, self.zy_height / 2.0+.5f)];
        [backPath addLineToPoint:CGPointMake(kPointWidth + kPointSpace + kAngleLength-1, self.zy_height / 2.0-0.5f)];
        [backPath closePath];
        [backPath stroke];
...
//        [backPath closePath];
        ...
        backPath.lineCapStyle = kCGLineCapRound;
        backPath.lineJoinStyle = kCGLineCapRound;
        [backPath moveToPoint:CGPointMake(self.zy_width - kPointWidth - kPointSpace, self.zy_height / 2.0-0.5f)];
        [backPath addLineToPoint:CGPointMake(self.zy_width - kPointWidth - kPointSpace, self.zy_height/2.f+.5f)];
        [backPath addLineToPoint:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace, self.zy_height/2.f+.5f)];
        [backPath addLineToPoint:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace, self.zy_height/2.f-0.5f)];
        [backPath closePath];
        [backPath stroke];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan
{
    ...
    switch (pan.state) {
        ...
        {
            CGPoint endPoint = CGPointMake(panPoint.x - self.panTmpPoint.x, panPoint.y - self.panTmpPoint.y);
            [self changeLocationWithGestureState:UIGestureRecognizerStateEnded
                                   locationPoint:endPoint];
            if (self.tagViewEndMove) {
                self.tagViewEndMove(endPoint);
            }
...
//    cka.values =   @[@1.2,   @0.9,@0.7,@0.9,@1.2];
...
//    [self.pointLayer addAnimation:cka forKey:@"cka"];
    ...
    cka2.values =   @[@3.0, @2.f, @1.0f, @2,  @3.f];
    //    cka2.keyTimes = @[@0.8, @0.3, @0.3, @0.3, @0.3, @1];
